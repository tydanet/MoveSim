...
from utils import read_data_from_file, get_workspace_logger
from train import generate_samples, train_epoch, pretrain_model
...
def main(opt):
    ...
    if opt.pretrain:
        ...
        torch.save(
            generator.state_dict(), DATA_PATH + "/%s/pretrain/generator.pth" % opt.data
        )
        torch.save(
            discriminator.state_dict(),
            DATA_PATH + "/%s/pretrain/discriminator.pth" % opt.data,
        )

    else:
        generator.load_state_dict(
            torch.load(DATA_PATH + "/%s/pretrain/generator.pth" % opt.data)
        )
        discriminator.load_state_dict(
            torch.load(DATA_PATH + "/%s/pretrain/discriminator.pth" % opt.data)
        )
        print("")
    ...
    for epoch in range(EPOCHS):
        ...
        print(
            "Current JSD: %f, %f, %f, %f, %f, %f"
            % (JSDs[0], JSDs[1], JSDs[2], JSDs[3], JSDs[4], JSDs[5])
        )
        ...
        logger.info(
            "Epoch [%d] Generator Loss: %f, Discriminator Loss: %f"
        ...
        if (epoch + 1) % 20 == 0:
            generate_samples(
                ...
                SEQ_LEN,
                GENERATED_NUM,
                DATA_PATH + "/%s/gene_epoch_%d.data" % (opt.data, epoch + 1),
            )
    ...
    print(
        "Test JSD: %f, %f, %f, %f, %f, %f"
        % (JSDs[0], JSDs[1], JSDs[2], JSDs[3], JSDs[4], JSDs[5])
    )

    torch.save(generator.state_dict(), DATA_PATH + "/%s/generator.pth" % opt.data)
    torch.save(
        discriminator.state_dict(), DATA_PATH + "/%s/discriminator.pth" % opt.data
    )
